...
export default function HoiDapM() {
    ...
    const handleFilterSubmit = async (item) => {
        ...
        try {
            ...
            const response = await axios.put('http://192.168.10.72:8888/updateHoiDap', updatedFormPut);
    ...
    return (
        <div className='mt-4 min-h-screen font-fontgg text-center'>
            ...
            <div className="flex  flex-row justify-around">
                ...
                <div className="flex w-2/3  py-10">
                    <input type="search" id="filterText"
                        className=" w-full p-2 text-sm text-gray-900 bg-gray-100 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        placeholder="Search CCCD, NAME..." onChange={handleFilterChange} onKeyPress={(e) => {
                            if (e.key === 'Enter') {
                                //    handleFilterSubmit();
                            }
                        }} />
                    <button className='w-28 bg-slate-300'
                        onClick={handleFilterSubmit} type="button"  >Tìm kiếm
                        </button>
            ...
            <h1 className='m-auto font-bold text-2xl p-3'>Danh sách hỏi đáp</h1>
